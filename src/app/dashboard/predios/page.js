...
import { getAllPredios, createPredio, updatePredio, deletePredio } from "@/lib/actions/predios";
import { getAllDNBs } from "@/lib/actions/dnbs";
import { useSession } from "next-auth/react";
import { Plus, Edit, Trash2, Building2, AlertTriangle, X } from "lucide-react";

const DEFAULT_PREDIO_FORM = { nome: "", dnb: "", ativo: true };

function PredioDialog({ open, onClose, onSuccess, onRefresh, predio, dnbs }) {
  const isEdit = !!predio;
  const formRef = useRef(null);
  const saveAndNextRef = useRef(false);
  const [formData, setFormData] = useState(DEFAULT_PREDIO_FORM);
  ...
  useEffect(() => {
    if (predio) {
      setFormData({
        nome: predio.nome || "",
        dnb: predio.dnb?._id || predio.dnb || "",
        ativo: predio.ativo !== undefined ? predio.ativo : true,
      });
    } else {
      setFormData({ nome: "", dnb: "", ativo: true });
    }
    setError("");
  }, [predio, open]);
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (isEdit) {
        await updatePredio(predio._id, formData);
      } else {
        await createPredio(formData);
      }
      if (isNext) {
        setFormData({ ...DEFAULT_PREDIO_FORM, dnb: formData.dnb });
        setSavedFlash(true);
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-mono font-bold text-base" style={{ color: "#00d4ff" }}>
            {isEdit ? "EDITAR PRÉDIO" : "NOVO PRÉDIO"}
          </h2>
        ...
        <form onSubmit={handleSubmit} ref={formRef} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex: Bloco A, Sede, Prédio 1"
              className="w-full h-10 px-3 rounded text-sm font-mono"
              style={inputStyle}
...
export default function PrediosPage() {
  ...
  async function loadData() {
    try {
      setLoading(true);
      const [prediosData, dnbsData] = await Promise.all([getAllPredios(), getAllDNBs()]);
  ...
  async function handleDelete(id, nome) {
    if (!confirm(`Excluir o prédio "${nome}"? Esta ação não pode ser desfeita.`)) return;
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "#64748b" }}>
            Gerencie os prédios e instalações por DNB
          </p>
        </div>
        {canManage && (
          <button
      ...
      {predios.length === 0 ? (
        ...
        >
          <Building2 className="h-12 w-12 mb-4" style={{ color: "#1a3a4a" }} />
              ...
              {sortedPredios.map((predio, idx) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm font-mono" style={{ color: "#64748b" }}>
                    {predio.dnb ? `${predio.dnb.code} — ${predio.dnb.name}` : "-"}
                  </td>
                  <td className="px-4 py-3">
                    ...
                    >
                      {predio.ativo ? "Ativo" : "Inativo"}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-sm" style={{ color: "#64748b" }}>
                    {predio.criadoPor?.name || "-"}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      {canManage && (
                        <button
                          onClick={() => { setSelected(predio); setDialogOpen(true); }}
                      ...
                      {canDelete && (
                        <button
                          onClick={() => handleDelete(predio._id, predio.nome)}
                          className="p-1.5 rounded transition-all"
                          style={{ color: "#64748b" }}

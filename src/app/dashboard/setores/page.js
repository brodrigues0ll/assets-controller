...
import { getAllSetores, createSetor, updateSetor, deleteSetor } from "@/lib/actions/setores";
import { getPredios } from "@/lib/actions/predios";
import { useSession } from "next-auth/react";
import { Plus, Edit, Trash2, MapPin, AlertTriangle, X } from "lucide-react";

const DEFAULT_SETOR_FORM = { nome: "", predio: "", descricao: "", ativo: true };

function SetorDialog({ open, onClose, onSuccess, onRefresh, setor, predios }) {
  const isEdit = !!setor;
  const formRef = useRef(null);
  const saveAndNextRef = useRef(false);
  const [formData, setFormData] = useState(DEFAULT_SETOR_FORM);
  ...
  useEffect(() => {
    if (setor) {
      setFormData({
        nome: setor.nome || "",
        predio: setor.predio?._id || setor.predio || "",
        descricao: setor.descricao || "",
        ativo: setor.ativo !== undefined ? setor.ativo : true,
      });
    } else {
      setFormData({ nome: "", predio: "", descricao: "", ativo: true });
    }
    setError("");
  }, [setor, open]);
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (isEdit) {
        await updateSetor(setor._id, formData);
      } else {
        await createSetor(formData);
      }
      if (isNext) {
        setFormData({ ...DEFAULT_SETOR_FORM, predio: formData.predio });
        setSavedFlash(true);
        ...
        <form onSubmit={handleSubmit} ref={formRef} className="space-y-4">
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData((p) => ({ ...p, predio: e.target.value }))}
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex: TI, Operacional, Sala 101"
              className="w-full h-10 px-3 rounded text-sm font-mono"
              style={inputStyle}
...
export default function SetoresPage() {
  ...
  async function loadData() {
    try {
      setLoading(true);
      const [setoresData, prediosData] = await Promise.all([
        getAllSetores(),
        getPredios(),
      ]);
  ...
  async function refreshSilent() {
    try {
      const [setoresData, prediosData] = await Promise.all([getAllSetores(), getPredios()]);
      setSetores(setoresData);
      setPredios(prediosData);
    } catch {}
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "#64748b" }}>
            Gerencie os setores e salas dos prédios
          </p>
        </div>
        {canManage && (
          <button
      ...
      {setores.length === 0 ? (
        ...
        >
          <MapPin className="h-12 w-12 mb-4" style={{ color: "#1a3a4a" }} />
          ...
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: "1px solid #1a3a4a" }}>
                ...
                <th
                  onClick={() => handleSort('predio.nome')}
                  className="px-4 py-3 text-left text-xs font-mono uppercase tracking-wider cursor-pointer select-none"
                  style={{ color: "#00d4ff", background: "#141428" }}
                  onMouseEnter={(e) => { e.currentTarget.style.color = "#e2e8f0"; }}
                  onMouseLeave={(e) => { e.currentTarget.style.color = "#00d4ff"; }}
                >
                  <span className="flex items-center gap-1">
                    Prédio
                    <span style={{ opacity: sortCol === 'predio.nome' ? 1 : 0.25, fontSize: '10px' }}>
                      {sortCol === 'predio.nome' && sortDir === 'desc' ? '▼' : '▲'}
                    </span>
                  </span>
                </th>
                <th
                  onClick={() => handleSort('predio.dnb.code')}
                  className="px-4 py-3 text-left text-xs font-mono uppercase tracking-wider cursor-pointer select-none"
                  style={{ color: "#00d4ff", background: "#141428" }}
                  onMouseEnter={(e) => { e.currentTarget.style.color = "#e2e8f0"; }}
                  onMouseLeave={(e) => { e.currentTarget.style.color = "#00d4ff"; }}
                >
                  <span className="flex items-center gap-1">
                    DNB
                    <span style={{ opacity: sortCol === 'predio.dnb.code' ? 1 : 0.25, fontSize: '10px' }}>
                      {sortCol === 'predio.dnb.code' && sortDir === 'desc' ? '▼' : '▲'}
                    </span>
                  </span>
                </th>
                <th
              ...
              {sortedSetores.map((setor, idx) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm" style={{ color: "#64748b" }}>
                    {setor.criadoPor?.name || "-"}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      {canManage && (
                        <button
                          onClick={() => { setSelected(setor); setDialogOpen(true); }}
                      ...
                      {canDelete && (
                        <button
                          onClick={() => handleDelete(setor._id, setor.nome)}
                          className="p-1.5 rounded transition-all"
                          style={{ color: "#64748b" }}
